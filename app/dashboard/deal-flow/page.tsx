...
export default async function DealFlowPage({
  ...
  // Load all FX rates once → fxMap for the shared scorer.
  const { data: fxRows } = await supabase.from("fx_rates").select("currency, rate_to_usd");
  const fxMap = buildFxMap(fxRows);
  ...
  const scored = projects.map(p => ({ ...p, _match: scoreMatch(p, investorProfile, fxMap).score, _saved: savedIds.has(p.id) }));
  ...
  return (
    <div className="p-6 md:p-8 pt-[68px] md:pt-8 max-w-5xl mx-auto">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="font-display text-2xl md:text-3xl font-bold text-[#22201B] mb-1.5">Deal Flow</h1>
        <p className="text-[#918A7C] text-sm">
          {projects.length
            ? <>
      ...
      {projects.length > 0 && (
        <div className="mb-6 flex flex-wrap items-center gap-2">
          <span className="text-[#918A7C] text-xs uppercase tracking-widest mr-2">Trier</span>
      ...
      {user && (!investorProfile || !investorProfile.priority_sectors?.length)
        ...
        <div className="mb-6 p-4 rounded-xl bg-[#1F4E79]/6 border border-[#1F4E79]/25 flex items-start gap-3 text-sm">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[#22201B]/75 font-medium mb-0.5">Activez la correspondance automatique</p>
      ...
      {sorted.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
          {sorted.map(project => (
            <div key={project.id}
              ...
              {/* Title + tagline */}
              <h3 className="text-[#22201B] font-semibold text-base mb-1.5 leading-snug">{project.title}</h3>
              {project.tagline && (
                <p className="text-[#918A7C] text-xs leading-relaxed mb-4 flex-1 line-clamp-3">{project.tagline}</p>
              )}
              ...
              <div className="border-t border-[#E4E7EC] pt-3 mt-3 flex flex-wrap items-center gap-2">
                ...
                {project.amount_requested && (
                  <span className="ml-auto text-[#575249] font-semibold text-sm tabular-nums">
        ...
        <div className="rounded-xl border border-[#E4E7EC] bg-white p-12 text-center max-w-md mx-auto">
          ...
          <h2 className="text-[#22201B] font-display text-lg font-bold mb-2">
            {savedOnly ? "Aucun projet sauvegardé" : "Aucune opportunité pour le moment"}
          </h2>
          <p className="text-[#918A7C] text-sm mb-6 leading-relaxed">
            {savedOnly
