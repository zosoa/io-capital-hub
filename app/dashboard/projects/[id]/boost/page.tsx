    ...
    <div className="card overflow-hidden">
      <div className={`px-6 py-4 border-b flex items-center justify-between gap-3 ${done ? "bg-green-50 border-green-100" : "bg-[#FAF7F3] border-[#EDE7DE]"}`}>
        <div className="flex items-center gap-2.5">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${done ? "bg-green-100 text-green-600" : "bg-[#1F4E79]/10 text-[#1F4E79]"}`}>
            {done ? (
        ...
        <div className="flex items-center gap-2">
          {saving    && <span className="text-[10px] text-[#1F4E79] animate-pulse">Sauvegarde…</span>}
          {!saving && lastSaved && <span className="text-[10px] text-green-600 flex items-center gap-1">
            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
            Sauvegardé
          </span>}
          <div className={`text-xs font-semibold px-2 py-0.5 rounded-full ${done ? "bg-green-100 text-green-700" : pct > 0 ? "bg-[#1F4E79]/10 text-[#1F4E79]" : "bg-[#F4F5F7] text-[#B0A898]"}`}>
            {pct}%
          </div>
        </div>
      </div>
        ...
        return (
          <button key={o.v} type="button" onClick={() => toggle(o.v)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-medium transition-all ${
              on ? "border-[#1F4E79] bg-[#1F4E79]/8 text-[#0F1320]" : "border-[#DDD8D0] text-[#7A8098] hover:border-[#1F4E79]/40"
            }`}>
            {on && <svg className="w-3 h-3 text-[#1F4E79]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>}
            {o.l}
          </button>
        );
      })}
      ...
      {options.map(o => (
        <button key={o.v} type="button" onClick={() => onChange(o.v)}
          className={`text-left p-3 rounded-xl border-2 transition-all ${
            value === o.v ? "border-[#1F4E79] bg-[#1F4E79]/6" : "border-[#E8E2D9] hover:border-[#1F4E79]/30"
          }`}>
...
function ToggleField({ checked, onChange, labelOn, labelOff }: {
  ...
  return (
    <div className="flex items-center gap-3 p-3.5 bg-[#FAF7F3] rounded-xl border border-[#E8E2D9] cursor-pointer"
      ...
      <div className={`w-10 h-5 rounded-full transition-all flex items-center px-0.5 flex-shrink-0 ${checked ? "bg-[#1F4E79]" : "bg-[#DDD8D0]"}`}>
  ...
  return (
    <div className="card overflow-hidden mb-5">
      <button
        ...
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-[#1F4E79]/10 text-[#1F4E79] flex items-center justify-center flex-shrink-0">
      ...
      {open && (
        <div className="p-6 space-y-5">
              ...
              return (
                <div key={cat.label} className="p-4 rounded-xl border border-[#E8E2D9] bg-[#FAF7F3]">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs font-bold tabular-nums ${pct === 100 ? "text-green-600" : "text-[#1F4E79]"}`}>
                      {earnedPts}/{cat.pts} pts
                    </span>
                  </div>
                  <div className="h-1 bg-[#E8E2D9] rounded-full overflow-hidden mb-3">
                    <div className={`h-full rounded-full transition-all duration-500 ${pct === 100 ? "bg-green-500" : "bg-[#1F4E79]"}`} style={{ width: `${pct}%` }}/>
                  </div>
                  <ul className="space-y-1">
                    {cat.items.map(item => (
                      <li key={item.label} className="flex items-center gap-2 text-[11px]">
                        ...
                        <span className={`ml-auto font-semibold tabular-nums flex-shrink-0 ${item.done ? "text-green-600" : "text-[#1F4E79]"}`}>
                          +{item.pts}
                        </span>
                      </li>
                    ))}
          ...
          {/* Next steps hint */}
          {missing.length > 0 && (
            <div className="p-4 rounded-xl bg-[#1F4E79]/6 border border-[#1F4E79]/20">
              <div className="text-xs font-bold text-[#1F4E79] uppercase tracking-wider mb-2">
                Pour gagner le plus de points rapidement
              </div>
              <ul className="space-y-1.5">
                {missing.slice(0, 3).map(item => (
                  <li key={item.label} className="flex items-center gap-2 text-xs text-[#5A6280]">
                    <svg className="w-3 h-3 text-[#1F4E79] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5"/>
                    </svg>
                    <span>{item.label}</span>
                    <span className="ml-auto font-semibold text-[#1F4E79]">+{item.pts} pts</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className="card p-6 mb-7 flex items-center gap-5">
        ...
        <div className="relative w-20 h-20 flex-shrink-0">
          <svg viewBox="0 0 36 36" className="w-20 h-20 -rotate-90">
            ...
            <circle cx="18" cy="18" r="15.5" fill="none" stroke="#1F4E79" strokeWidth="3"
        ...
        <div className="flex-1">
          ...
          <div className="mt-3 h-1.5 bg-[#EDE7DE] rounded-full overflow-hidden">
            <div className="h-full bg-[#1F4E79] rounded-full transition-all duration-700" style={{ width: `${globalScore}%` }}/>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div className="card overflow-hidden">
          <div className="px-6 py-4 border-b border-[#EDE7DE] bg-[#FAF7F3] flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-[#1F4E79]/10 text-[#1F4E79] flex items-center justify-center">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="w-20 h-20 rounded-2xl border-2 border-[#E8E2D9] bg-[#FAF7F3] flex items-center justify-center overflow-hidden flex-shrink-0">
                {logoUrl ? (
                  ...
                  <span className="text-[#1F4E79] font-bold text-2xl">{project?.title?.charAt(0).toUpperCase()}</span>
                )}
              </div>
              {/* Upload */}
              <div className="flex-1">
                ...
                <label className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium cursor-pointer transition-all ${
                  logoUploading ? "opacity-50 cursor-not-allowed" : "border-[#1F4E79]/30 text-[#1F4E79] hover:bg-[#1F4E79]/5"
                }`}>
                  {logoUploading ? (
                    <>
                      <span className="w-3.5 h-3.5 border-2 border-[#1F4E79]/30 border-t-[#1F4E79] rounded-full animate-spin"/>
                      Envoi en cours…
                    </>
                  ) : (
                    <>
        ...
        >
          ...
          {isEquity && (
            <div className="pt-2 border-t border-[#EDE7DE]">
              <div className="text-xs font-bold text-[#1F4E79] uppercase tracking-wider mb-4">
              ...
              return (
                <button key={d.v} type="button"
                  ...
                  className={`flex items-center gap-2.5 p-3 rounded-xl border text-left transition-all ${
                    on ? "border-[#1F4E79] bg-[#1F4E79]/6" : "border-[#E8E2D9] hover:border-[#1F4E79]/30"
                  }`}>
                  <div className={`w-4 h-4 rounded border-2 flex-shrink-0 flex items-center justify-center transition-all ${on ? "border-[#1F4E79] bg-[#1F4E79]" : "border-[#C8C0B5]"}`}>
