...
// ─── Funding types — simplified terminology ───────────────────
const FUNDING_TYPES = [
  {
    v: "debt",
    l: "Emprunt / Prêt",
...
const LEGAL_NOTES: Record<string, string> = {
  ...
  "Maurice":      "À Maurice : les formes Ltd, SA et GBC sont largement reconnues. Choisissez « Autre » si besoin.",
  "Réunion":      "À La Réunion (France) : SARL, SAS, SA et EI sont disponibles.",
  "Comores":      "Aux Comores : SARL et SA sont les formes dominantes.",
  "Seychelles":   "Aux Seychelles : Ltd, SA et IBC sont courants. Choisissez « Autre » si besoin.",
...
// Rich funding-type card with expandable pros/cons
function FundingTypeCard({ type, selected, expanded, onSelect, onToggleExpand }: {
  type: typeof FUNDING_TYPES[0];
  selected: boolean;
  expanded: boolean;
  onSelect: () => void;
      ...
      <button type="button" onClick={onSelect} className="text-left w-full p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className={`font-semibold text-sm leading-none mb-1 ${selected ? "text-[#22201B]" : "text-[#575249]"}`}>
              {type.l}
            </div>
            <div className={`text-xs leading-snug ${selected ? "text-[#575249]" : "text-[#918A7C]"}`}>
              {type.tag}
            </div>
          </div>
        </div>
...
const EMPTY_FORM: ProjectFormData = {
  title:"", tagline:"", description:"", sector:"", sub_sector:"", stage:"", country:"Madagascar", city:"",
  funding_type:"", amount_requested:"", currency:"USD", funding_term_months:"", funding_duration_range:"",
  use_of_funds:"", investor_type_sought:[], equity_stake_offered:"", pre_money_valuation:"", exit_horizon:"",
  legal_structure:"", years_in_operation:"0", team_size:"", has_existing_revenue:false, annual_revenue:"",
  has_collateral:false, collateral_type:"", collateral_description:"", collateral_value:"",
  job_creation_expected:"", impact_description:"",
};
...
export default function NewProjectPage() {
  ...
  const [sectorOtherDesc, setSectorOtherDesc] = useState("");

  // Step 2
  const [expandedFundingType, setExpandedFundingType] = useState<string | null>(null);

  // Step 3
  const [legalStructureOther, setLegalStructureOther] = useState("");
  ...
  // Pre-onboarding handoff — hydrate the wizard from the /eligibilite funnel
  // (stored in sessionStorage by the eligibility check), then clear it so it
  // only pre-fills once.
  useEffect(() => {
    try {
      ...
      const SECTOR_MAP: Record<string, string> = {
        energy: "energy", agriculture: "agriculture", tech: "tech", tourism: "tourism",
        infrastructure: "infrastructure", education: "education", blue_economy: "other", mining: "other",
      };
      ...
      setForm(f => ({
        ...
        funding_duration_range: typeof lead.funding_duration_range === "string" ? lead.funding_duration_range : f.funding_duration_range,
        stage:                  typeof lead.stage === "string" ? lead.stage : f.stage,
        country:                typeof lead.country === "string" && COUNTRIES.includes(lead.country) ? lead.country : f.country,
      }));
      setPrefilled(true);
      sessionStorage.removeItem("capitalhub_lead");
    } catch { /* private mode / bad JSON — ignore */ }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // F3 — Warn before unload if form has data
  useEffect(() => {
    function handleBeforeUnload(e: BeforeUnloadEvent) {
      if (form.title.trim() || selectedSectors.length > 0) {
        e.preventDefault();
      }
    }
    window.addEventListener("beforeunload", handleBeforeUnload);
    return () => window.removeEventListener("beforeunload", handleBeforeUnload);
  }, [form.title, selectedSectors]);
  ...
  // Does at least one selected collateral type expect a monetary value?
  const hasValuedCollateral = selectedCollateralTypes.some(v =>
    COLLATERAL_TYPES.find(c => c.v === v)?.hasValue
  );
  ...
  function validate(): string {
    ...
    if (step === 2) {
      ...
      if (isNaN(amt) || amt <= 0)       return "Le montant doit être supérieur à 0.";
      if (amt > 500_000_000)            return "Le montant ne peut pas dépasser 500 000 000. Contactez-nous pour les projets de grande envergure.";
  ...
  async function submitProject(status: "draft" | "submitted" = "submitted") {
    setLoading(true); setError("");
    const supabase = createClient();
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { setError("Session expirée. Veuillez vous reconnecter."); setLoading(false); return; }

    // F11 — Block submission if the user hasn't confirmed they are the authorised representative
    if (status === "submitted") {
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-[#F6F7F9] border-b border-[#E4E7EC] px-4 sm:px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="text-[#575249] font-medium text-sm hidden sm:block">Soumettre un dossier</div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="glass-card rounded-2xl p-6 sm:p-8 border border-[#E4E7EC]">
          ...
          {step === 1 && (
            <div className="space-y-6">
              <FormField label="Titre du dossier *" hint="Soyez précis et accrocheur. Ex : « Ferme solaire 5 MW — Région Atsimo Andrefana »">
              ...
              {/* Stages — simplified labels + inline descriptions */}
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField label="Pays" hint={prefilled ? "Pré-rempli depuis votre profil · modifiable" : undefined}>
                  <select value={form.country} onChange={e => { update("country", e.target.value); setPrefilled(false); }} className="form-input">
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-[#918A7C] text-xs mb-3 leading-relaxed">
                  Cliquez sur &ldquo;En savoir plus&rdquo; pour comprendre les implications de chaque type avant de choisir.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {FUNDING_TYPES.map(f => (
                    <FundingTypeCard
                      key={f.v}
                      type={f}
                      selected={form.funding_type === f.v}
                      expanded={expandedFundingType === f.v}
                      onSelect={() => update("funding_type", f.v)}
                      onToggleExpand={e => {
                        e.stopPropagation();
                        setExpandedFundingType(prev => prev === f.v ? null : f.v);
                      }}
                    />
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <FormField label="Montant recherché *" hint="Aucun plafond — indiquez le montant exact">
                  <AmountInput
                    value={form.amount_requested}
                    onChange={v => update("amount_requested", v)}
              ...
              <div>
                ...
                <p className="text-[#8A8275] text-xs mt-2">Vous pouvez aussi préciser le nombre de mois si besoin :</p>
              ...
              {form.funding_type === "equity" && (
                <div className="p-5 bg-[#F6F7F9] rounded-xl border border-[#BC5A34]/20 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <FormField label="Valorisation pré-money" hint={`Estimation actuelle de l'entreprise (${form.currency})`}>
                      <AmountInput
                        value={form.pre_money_valuation}
                        onChange={v => update("pre_money_valuation", v)}
                        currencySymbol={CURRENCY_SYMBOLS[form.currency] || form.currency}
          ...
          {step === 3 && (
            <div className="space-y-6">
              {/* Legal structure — simplified with descriptions */}
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField label="Années d'existence">
                  <input type="number" value={form.years_in_operation} onChange={e => update("years_in_operation", e.target.value)}
              ...
              <FormField label="Revenus existants">
                <Toggle
                  checked={form.has_existing_revenue}
                  onChange={v => update("has_existing_revenue", v)}
              ...
              {form.has_existing_revenue && (
                <FormField label={`Chiffre d'affaires annuel (${form.currency})`} hint="Dernière année fiscale disponible">
                  <AmountInput
                    ...
                    onChange={v => update("annual_revenue", v)}
                    currencySymbol={CURRENCY_SYMBOLS[form.currency] || form.currency}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#918A7C] text-xs leading-relaxed">
                      Les investisseurs regardent l&apos;ensemble de votre dossier — vos actifs, mais aussi votre réputation, vos contrats et votre clientèle fidèle sont de véritables atouts. Cochez tout ce qui vous correspond.
                    </p>
                  </div>
                </div>
              </div>

              {/* Multi-select collateral types */}
              <div>
                ...
                {/* Nothing selected nudge */}
                {selectedCollateralTypes.length === 0 && (
                  <p className="text-[#8A8275] text-xs mt-3 leading-relaxed">
                    Si vous n&apos;avez aucune garantie formelle à ce stade, vous pouvez passer à l&apos;étape suivante — votre dossier sera tout de même soumis et évalué.
                  </p>
                )}
              </div>

              {/* Description (shown when any type selected) */}
              {selectedCollateralTypes.length > 0 && (
                <FormField label="Décrivez vos atouts" hint="Plus vous êtes précis, plus votre dossier inspire confiance aux investisseurs">
                  <textarea value={form.collateral_description} onChange={e => update("collateral_description", e.target.value)}
                    className="form-input resize-y" rows={3}
                    placeholder="Ex : Contrats signés avec 3 distributeurs représentant 200k€/an de CA récurrent, bureau en propriété estimé à 150k€..."/>
                </FormField>
              )}

              {/* Monetary value — only for asset-type collateral */}
              {hasValuedCollateral && (
            ...
            <div className="space-y-6">
              <div className="p-5 bg-[#F6F7F9] rounded-xl border border-[#E4E7EC]">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-[#22201B] font-semibold text-sm mb-0.5">Impact & Développement durable</h3>
                    <p className="text-[#918A7C] text-xs leading-relaxed">Les investisseurs de l&apos;Océan Indien sont de plus en plus attentifs à l&apos;impact économique, social et environnemental des projets financés.</p>
                  </div>
                </div>
              </div>

              <FormField label="Emplois créés (estimation)" hint="Nombre d'emplois directs prévus dans les 3 ans suivant le financement">
                <input type="number" value={form.job_creation_expected} onChange={e => update("job_creation_expected", e.target.value)}
              ...
              <div className="p-4 bg-[#F6F7F9] rounded-xl border border-[#E4E7EC] text-sm text-[#918A7C] leading-relaxed">
        ...
        <div className="fixed bottom-0 inset-x-0 z-40 bg-white backdrop-blur border-t border-[#E4E7EC] px-4 py-3
          ...
          <div className="max-w-4xl mx-auto flex items-center justify-between gap-3">
            ...
            <div className="text-[#6B655B] text-xs md:text-sm tabular-nums flex-shrink-0">{step} / {STEPS.length}</div>
