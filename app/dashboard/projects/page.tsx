    ...
    <div className="p-6 md:p-8 pt-[68px] md:pt-8 max-w-5xl mx-auto">
      ...
      {(!projects || projects.length === 0) ? (
        <div className="card p-12 text-center">
          <div className="w-12 h-12 bg-[#1F4E79]/8 border border-[#1F4E79]/20 rounded-xl flex items-center justify-center mx-auto mb-4">
            <svg className="w-6 h-6 text-[#1F4E79]/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          {projects.map(p => {
            ...
            return (
              <Link key={p.id} href={`/dashboard/projects/${p.id}`}
                className="block card hover:border-[#1F4E79]/40 hover:shadow-sm transition-all duration-200 group overflow-hidden">
                <div className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3.5 flex-1 min-w-0">
                      {/* Logo badge */}
                      <div className="w-11 h-11 rounded-xl border-2 border-[#E8D9B5] bg-[#1F4E79]/8 flex items-center justify-center flex-shrink-0 overflow-hidden mt-0.5">
                        {p.project_logo_url ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={p.project_logo_url} alt={p.title} className="w-full h-full object-cover"/>
                        ) : (
                          <span className="text-[#1F4E79] font-bold text-base">{p.title.charAt(0).toUpperCase()}</span>
                        )}
                      </div>
                      ...
                      <h3 className="text-[#0F1320] font-semibold group-hover:text-[#1F4E79] transition-colors truncate">
                        {p.title}
                      </h3>
                      {p.tagline && (
                    ...
                    <div className="text-right flex-shrink-0 flex flex-col items-end gap-2">
                      ...
                      {(p.boost_score > 0) && (
                        <div className="flex items-center gap-1 text-[10px] text-[#1F4E79] font-semibold">
                          <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
