...
export default async function AdminPage() {
  ...
  const { count: pendingIntros } = await supabase.from("deal_interests").select("id", { count: "exact", head: true }).eq("status", "pending");
      ...
      {(pendingIntros ?? 0) > 0 && (
        <Link href="/admin/intros?status=pending"
          ...
          <div className="flex-1">
            <div className="text-white text-sm font-medium">{pendingIntros} introduction{(pendingIntros ?? 0) > 1 ? "s" : ""} à faciliter</div>
            <div className="text-gray-400 text-xs">Des investisseurs attendent une mise en relation →</div>
          </div>
          <span className="bg-brand-gold text-brand-navy text-xs font-bold rounded-full px-2 py-0.5">{pendingIntros}</span>
        </Link>
      )}
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            Action requise
            {pendingProjects.length > 0 && <span className="bg-brand-gold text-brand-navy text-xs font-bold rounded-full px-2 py-0.5">{pendingProjects.length}</span>}
          </h2>
          <Link href="/admin/projects" className="text-brand-gold hover:text-[#C8992A] text-sm transition-colors">Voir tout →</Link>
        </div>

        {pendingProjects.length === 0 ? (
          <div className="glass-card rounded-xl p-6 text-center text-gray-400 text-sm border border-white/8 flex items-center justify-center gap-2">
